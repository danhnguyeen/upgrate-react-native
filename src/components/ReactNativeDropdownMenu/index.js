...
import { fontSize, platform } from '../../config/variables';

class DropdownMenu extends Component {
  constructor(props, context) {
    super(props, context);
    let dataFilter = [];
    dataFilter.push(this.props.businesses);
  ...
  renderActivityPanel() {
    if (this.state.activityIndex >= 0) {
      ...
      return (
        <Animatable.View style={{position: 'absolute', left: 20, right: 20, top: 40, bottom: 0}}>
          <ScrollView
  ...
  itemOnPress(index) {
    if (this.state.activityIndex > -1) {
      ...
      if (this.props.handler) {
        this.props.handler(this.state.activityIndex, index, this.state.dataFilter);
      }
    }
  ...
  render() {
    return (
      <View style={{flexDirection: 'column', flex: 1, paddingTop: 5, paddingLeft: 20, paddingRight: 20}}>
        <View style={{
          flexDirection: 'row',
          borderRadius: 5,
          borderWidth: 0.5,
              ...
              <TouchableOpacity
                ...
                <View style={{ alignItems: "center", justifyContent: "space-between" }}>
                  <Text
                    ...
                    {rows[this.state.selectIndex[index]] ? rows[this.state.selectIndex[index]].BrandName : ''}
                  </Text>
                </View>
